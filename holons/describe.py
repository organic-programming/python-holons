...
from holons.identity import HolonIdentity, parse_holon

_GENERATED_PYTHON_DIR = Path(__file__).resolve().parents[1] / "gen" / "python"
if str(_GENERATED_PYTHON_DIR) not in sys.path:
    sys.path.insert(0, str(_GENERATED_PYTHON_DIR))

from holonmeta.v1 import holonmeta_pb2, holonmeta_pb2_grpc

HOLONMETA_SERVICE_NAME = "holonmeta.v1.HolonMeta"
...
class _HolonMetaServicer(holonmeta_pb2_grpc.HolonMetaServicer):
    def __init__(self, response: holonmeta_pb2.DescribeResponse):
        self._response = response

    def Describe(
        self,
        _request: holonmeta_pb2.DescribeRequest,
        _context: grpc.ServicerContext,
    ) -> holonmeta_pb2.DescribeResponse:
...
def build_response(
    proto_dir: str | Path,
    holon_yaml_path: str | Path,
) -> holonmeta_pb2.DescribeResponse:
    """Parse holon identity and local protos into a Describe response."""
    identity = parse_holon(holon_yaml_path)
    services = _parse_services(proto_dir)
    return holonmeta_pb2.DescribeResponse(
...
def register(
    ...
    """Register the HolonMeta gRPC service on a server."""
    if server is None:
        raise ValueError("grpc server is required")

    response = build_response(proto_dir, holon_yaml_path)
    holonmeta_pb2_grpc.add_HolonMetaServicer_to_server(_HolonMetaServicer(response), server)


def _parse_services(proto_dir: str | Path) -> list[holonmeta_pb2.ServiceDoc]:
...
class _ResponseBuilder:
    ...
    def build_services(self, rel_files: list[str]) -> list[holonmeta_pb2.ServiceDoc]:
        services: list[holonmeta_pb2.ServiceDoc] = []
        for file_name in rel_files:
            ...
            for service_index, service in enumerate(file_proto.service):
                ...
                if service_name == HOLONMETA_SERVICE_NAME:
                    continue
    ...
    def _build_method(
        ...
        return holonmeta_pb2.MethodDoc(
            name=method.name,
            description=meta.description,
    ...
    def _build_enum_values(self, enum_type: _EnumRef) -> list[holonmeta_pb2.EnumValueDoc]:
        values: list[holonmeta_pb2.EnumValueDoc] = []
    ...
    def _field_label(self, field: descriptor_pb2.FieldDescriptorProto) -> int:
        if self._is_map_field(field):
            return holonmeta_pb2.FIELD_LABEL_MAP
        if field.label == descriptor_pb2.FieldDescriptorProto.LABEL_REPEATED:
            return holonmeta_pb2.FIELD_LABEL_REPEATED
        return holonmeta_pb2.FIELD_LABEL_OPTIONAL
